...
-- V20: Süresi dolan açık artırmaları güvenli ve tekrar çalıştırılabilir biçimde sonuçlandırır.
...
begin
  ...
  loop
    ...
    if v_bid.id is not null and v_bid.amount >= v_auction.reserve_price then
      update public.auctions
      set status = 'sold', winner_id = v_bid.bidder_id
      where id = v_auction.id;

      insert into public.payments (
        auction_id, buyer_id, seller_id, amount, commission_amount, status
      ) values (
        v_auction.id,
        v_bid.bidder_id,
        v_auction.seller_id,
        v_bid.amount,
        round(v_bid.amount * 0.08, 2),
        'pending'
      ) on conflict (auction_id) do nothing;

      insert into public.shipments (
        auction_id, buyer_id, seller_id, status
      ) values (
        v_auction.id,
        v_bid.bidder_id,
        v_auction.seller_id,
      ...
      insert into public.notifications (user_id, auction_id, title, body)
      select v_bid.bidder_id, v_auction.id, 'Açık artırmayı kazandın',
             v_auction.title || ' için ödeme süreci başladı.'
      where not exists (
        select 1 from public.notifications n
        where n.user_id = v_bid.bidder_id
          and n.auction_id = v_auction.id
          and n.title = 'Açık artırmayı kazandın'
      );

      insert into public.notifications (user_id, auction_id, title, body)
      select v_auction.seller_id, v_auction.id, 'İlanın satıldı',
             v_auction.title || ' ' || v_bid.amount::text || ' TL ile tamamlandı.'
      where not exists (
        select 1 from public.notifications n
        where n.user_id = v_auction.seller_id
          and n.auction_id = v_auction.id
          and n.title = 'İlanın satıldı'
      );
    else
      update public.auctions
      set status = 'ended', winner_id = null
      where id = v_auction.id;

      insert into public.notifications (user_id, auction_id, title, body)
      select v_auction.seller_id, v_auction.id, 'Açık artırma sona erdi',
             case
               when v_bid.id is null then v_auction.title || ' teklif almadan sona erdi.'
               else v_auction.title || ' için taban fiyat karşılanmadı.'
             end
      where not exists (
        select 1 from public.notifications n
        where n.user_id = v_auction.seller_id
          and n.auction_id = v_auction.id
          and n.title = 'Açık artırma sona erdi'
      );
    end if;
